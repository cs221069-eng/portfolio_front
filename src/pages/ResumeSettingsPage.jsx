...
export default function ResumeSettingsPage({ mode = 'resume' }) {
  ...
  const fetchResume = async () => {
    try {
      ...
      setResume(null)
      setStatus(error.response?.data?.message || 'Unable to load the current resume.')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setStatus(error.response?.data?.message || 'Unable to upload the resume.')
    } finally {
  ...
  const handleDelete = async () => {
    try {
      setIsDeleting(true)
      const response = await axios.delete(`${API_BASE_URL}/api/resume/delete`)
      setResume(null)
      setResumeFile(null)
      setStatus(response.data.message || 'Resume deleted successfully.')
    } catch (error) {
      setStatus(error.response?.data?.message || 'Unable to delete the resume.')
    } finally {
      setIsDeleting(false)
    }
  }
  ...
  return (
    <div className="page-stack">
      ...
      {!isSettings ? (
        <Panel eyebrow="Section / Resume Management" title={resume?.fileName || 'No Resume Uploaded'}>
          <div className="resume-card">
            <MaterialIcon name="description" className="resume-icon" />
            <div className="resume-copy">
              <strong>{resume ? 'Current resume is ready for download' : 'No resume has been uploaded yet'}</strong>
              <p>
                {isLoading
          ...
          <div className="button-row">
            <button type="button" className="primary-button" onClick={handleUpload} disabled={isUploading}>
              {isUploading ? 'Uploading...' : resume ? 'Replace Resume' : 'Upload Resume'}
            </button>
            <button type="button" className="ghost-button" onClick={() => window.open(`${API_BASE_URL}/api/resume/download`, '_blank')}>
              Download Test
            </button>
            <button type="button" className="ghost-button danger-outline" onClick={handleDelete} disabled={!resume || isDeleting}>
      ...
      <Panel eyebrow="Section / Security" title="Security & System">
        ...
        <button type="button" className="danger-card">
          <div>
            ...
            <p>Force logout from all signed-in devices and reset the current admin token.</p>
          </div>
